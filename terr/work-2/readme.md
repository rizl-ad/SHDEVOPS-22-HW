...
## Задача 4
Не совсем понял что значит фраза "(без хардкода!!!)" - создание ВМ мы захардкодили - объявили двумя ресурсами - поэтому output по ним также придется хардкодить, т.е. указывать для каждой ВМ отдельные строки.  
