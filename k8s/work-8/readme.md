...
- наличие минимум трёх worker-нод позволит обеспечить отказоустойчивость при выходе из строя одной из нод.
...
Далее к этим значениям следует прибавить ресурсы, потребляемые системными компонентами:
...
### Сравнение с кластером из четырёх worker-нод
...
- 3 ноды 12 vCPU и 24 ГБ RAM – цена аренды на момент написания отчёта **15 467,11** руб/мес, за три ноды = **15 467,11 x 3 = 46 401,33** руб/мес
...
Исходя из указанных нагрузок для обеспечения HA считаем достаточным минимальных трёх master-нод c размещенным на них etcd, и выделением следующих ресурсов на каждую ноду:
- 2 CPU cores/vCPU
- 4 ГБ RAM
