# Рещение ДЗ к занятию 4 «Оркестрация группой Docker контейнеров на примере Docker Compose»
## Задача 1
...
## Задача 3
#### п. 1-5
Для подключения к стандартному потоку ввода/вывода/ошибок контейнера используется команда ```docker attach```.
По п.3: при нажатии сочетания клавиш ```Ctrl+c``` контейнеру отправляется сигнал ```SIGINT```, останавливающий контейнер.
...
Метод смены порта без создания нового контейнера такой себе - подходит если на хосте запущен только один контейнер.
